...
int main(int argc, char** argv){
    ...
    if(window == NULL){
        cerr << "Could not initialize window \n" << SDL_GetError() << endl; 
        exit(1);
    }
    ...
    if(renderer == NULL){
        cerr << "Could not create Renderer! \n" << SDL_GetError() << endl;
        exit(1);
    } 
    ...
    int timer = 0;
    int start = SDL_GetTicks(); //for measuring average FPS
    int start_per_frame = SDL_GetTicks(); //for capping FPS
