...
void CHIP8::intialise(){
    ...
    //set the value of each pixel in font
    for (int i = 0; i < 80; i++)
        memory[i] = font[i];
...
bool CHIP8::loadROM(const string &filename){
    ifstream file;
    cerr << "Loading program: " << filename << endl;
    ...
    //check if there is space in the RAM
    if(length < 4096 - 512){
        for (int i = 0; i < length; ++i)
            memory[512 + i] = buffer[i];
    }else{
        cerr << "Program is too big" << endl;
        file.close();
        return 1;
    }
